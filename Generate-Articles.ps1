...
foreach ($keyword in $keywords) {
    $slug = $keyword.ToLower() -replace '[^a-z0-9 ]', '' -replace ' ', '-'
    $title = (Get-Culture).TextInfo.ToTitleCase($keyword)
    
    $filePath = "articles/$slug.html"
    
    Write-Host "Generating optimized content for: $title"
    ...
    $currentApiKey = $apiKeys[$keyIndex % $apiKeys.Count]
    $keyIndex++
    ...
    while ($retries -gt 0 -and -not $success) {
        try {
            ...
            Start-Sleep -Seconds 1 # small delay for rate limit
        }
        catch {
            Write-Host "Error generating $title, Retrying... ($retries left)" -ForegroundColor Red
            $retries--
            Start-Sleep -Seconds 2
        }
    }
}
Write-Host "All 91 Articles have been completely regenerated with the new content!"
